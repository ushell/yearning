...
func main() {
	flag.Parse()
	// 管理员密码
	var adminPassword = os.Getenv("YEARNING_ADMIN_PASSWORD")
	...
	if h {
		...
		if f {
			model.DB().Model(model.CoreAccount{}).Where("username =?", "admin").Update(&model.CoreAccount{Password: lib.DjangoEncrypt(adminPassword, string(lib.GetRandom()))})
			fmt.Println("admin密码已重新设置为:", adminPassword)
		}
	}
